...
export default function Home() {
  ...
  const [view, setView] = useState('dashboard'); // 'dashboard' | 'network' | 'admin'
  ...
  return (
    <div className="flex min-h-screen w-full bg-[#050505] text-zinc-300 font-sans selection:bg-indigo-500/30">
      ...
      {/* MAIN CONTENT */}
      <main className="flex-1 ml-16 lg:ml-64 p-6 lg:p-10 max-w-7xl mx-auto w-full">
...
const NavBtn = ({ label, active, onClick, icon: Icon }) => (
  <button
    onClick={onClick}
    className={`flex items-center justify-center lg:justify-start w-full p-3 rounded-md transition-all duration-200 ${
      active 
        ...
        {messages.map((m, i) => {
          ...
          return (
            <div key={i} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} animate-in slide-in-from-bottom-2 duration-300`}>
              ...
              <div className={`px-4 py-2 rounded-lg text-sm max-w-[85%] break-words shadow-lg ${
                isMe 
