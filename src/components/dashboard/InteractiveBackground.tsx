...
export default function InteractiveBackground() {
    ...
    const draw = () => {
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (dist < 100) {
            ctx.beginPath();
            ctx.strokeStyle = `rgba(94, 234, 212, ${0.1 * (1 - dist / 100)})`;
            ctx.lineWidth = 1;
            ctx.moveTo(rx1, ry1);
            ctx.lineTo(rx2, ry2);
            ctx.stroke();
  ...
  return (
    <div className="fixed inset-0 z-[-1] overflow-hidden bg-[#030607]">
      ...
      <div className="absolute top-0 inset-x-0 h-40 bg-gradient-to-b from-[#030607] to-transparent pointer-events-none" />
